...
class DatasetPASCAL(Dataset):
    ...
    def build_img_metadata(self):
        ...
        def read_metadata(split, fold_id):
            fold_n_metadata = os.path.join('/data16tb/llq/Code/ICL_diversity/data/splits/pascal/%s/fold%d.txt' % (split, fold_id))
        ...
        if os.path.exists("/data16tb/llq/Code/ICL_diversity/data/splits/pascal/%s/fold%d_ins.json" % (self.split, self.fold)):
            with open("/data16tb/llq/Code/ICL_diversity/data/splits/pascal/%s/fold%d_ins.json"%(self.split, self.fold), 'r', encoding='utf-8') as fp:
                img_metadata = json.load(fp)
        else:
            all_instance = os.listdir('/data16tb/llq/Code/ICL_diversity/datasets/pascal-5i/VOC2012/SegmentationObject')
            ...
            for metadata in img_metadata:
                if metadata[0] in all_instance:
                    ...
                    query_ins_cmask = Image.open(
                        os.path.join('/data16tb/llq/Code/ICL_diversity/datasets/pascal-5i/VOC2012/SegmentationObject',
                                     metadata[0]) + '.png')
            ...
            with open("/data16tb/llq/Code/ICL_diversity/data/splits/pascal/%s/fold%d_ins.json" % (self.split, self.fold), 'w', encoding='utf-8') as fp:
