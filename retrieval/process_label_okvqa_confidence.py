...
question_to_image_id = {q['question_id']: q['image_id'] for q in questions_data}

# 创建查找字典 - 使用question_id作为键
questions_dict = {q['question_id']: q['question'] for q in questions_data}
answers_dict = {a['question_id']: a['answers'] for a in answers_data}

# 创建新的数据集列表和查询示例字典
new_dataset = []
# 修改数据结构，使用 question_id 而不是 image_id
...
for pair in tqdm(pair_data['samples'], desc="收集示例"):
    ...
    # 直接使用 question_id
    query_examples[query_question_id].append({
        'example_question_id': example_question_id,
        'example_image': f'COCO_train2014_{str(question_to_image_id[example_question_id]).zfill(12)}.jpg',
        ...
        'label': pair['confidence_score'],  # 重命名 label 为 confidence_score
        'similarity_score': pair['similarity_score']   # 添加 similarity_score
    })
...
#         reverse=True
#     )
#     query_examples[query_id] = sorted_examples[:4] + sorted_examples[-4:]
...
# 构建新的数据集
for query_question_id, examples in tqdm(query_examples.items(), desc="构建数据集"):
    if examples:
    # if query_question_id in selected_queries and examples:
        query_image_id = question_to_image_id[query_question_id]
        new_dataset.append({
            'query_question_id': query_question_id,
            'query_image': f'COCO_train2014_{str(query_image_id).zfill(12)}.jpg',
            'query_question': questions_dict.get(query_question_id, ""),
            'query_answers': [a["answer"] for a in answers_dict.get(query_question_id, [])],
            'examples': examples
        })
