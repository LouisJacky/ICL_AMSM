...
for query_image in selected_query_images:
    ...
    all_samples_num += (len(positive_samples) + len(negative_samples))
    ...
    for _ in range(num_pairs):
        ...
        new_dataset.append({
            ...
            'example_question': positive_question,
            'example_answers': positive_answers,
            'label': 1  # 正样本标记
        })
        ...
        new_dataset.append({
            ...
            'query_question': query_question,
            'query_answers': query_answers,
            'example_image': negative_image,
            'example_question': negative_question,
            'example_answers': negative_answers,
            'label': 0  # 负样本标记
        })
...
output_file = '/data16tb/ljq/Code/ICL_diversity_ofv3/data/vizwiz_labeled_new_dataset.json'
with open(output_file, 'w') as f:
    json.dump(new_dataset, f, indent=2)

print(f"已将随机打乱顺序后的配对数据集保存到 {output_file}")
print(f"原总样本数：{all_samples_num}，保留总样本数：{len(new_dataset)}，正样本数：{positive_samples_num}，负样本数：{negative_samples_num}")
