...
@torch.no_grad()
def run_one_image(img, tgt, model, device, need_flops=False, need_vis=False):
    x = torch.tensor(img)
    ...
    loss, y, mask, latent_target = model(x.float().to(device), tgt.float().to(device), bool_masked_pos.to(device),
                                      valid.float().to(device), seg_type.to(device), feat_ensemble)
    if need_flops:
        seg_flops, seg_par = profile(model,
                                     inputs=(x.float().to(device), tgt.float().to(device), bool_masked_pos.to(device),
                                             valid.float().to(device), seg_type.to(device), feat_ensemble,))
    ...
    if need_flops:
        return output, latent_target, seg_flops, seg_par
    if need_vis:
        return loss, output, latent_target
    return output, latent_target


def inference_image(model, device, img_path, img2_paths, tgt2_paths, need_flops=False, need_vis=False):
    res, hres = 448, 448
    ...
    torch.manual_seed(2)
    if need_flops:
        raw_output, latent_target, seg_flops, seg_par = run_one_image(img, tgt, model, device, need_flops)
    elif need_vis:
        loss, raw_output, latent_target = run_one_image(img, tgt, model, device, need_vis=need_vis)
    else:
        raw_output, latent_target = run_one_image(img, tgt, model, device)
    ...
    if need_flops:
        return output_mask, output, raw_output, latent_target, seg_flops, seg_par
    if need_vis:
        return loss, output_mask, output, raw_output, latent_target
