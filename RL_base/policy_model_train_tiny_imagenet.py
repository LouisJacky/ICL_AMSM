...
class ClassificationDataset(Dataset):
    def __init__(self, json_file, image_dir, num_pairs=20, split="train"):
    ...
    def __getitem__(self, idx):
        ...
        # 加载query图像(只需加载一次)
        query_image = Image.open(os.path.join(self.image_dir, self.split, item['query_image']))

        for example in selected_examples:
            example_image = Image.open(os.path.join(self.image_dir, self.split, example['example_image']))
            query_images.append(query_image)
            example_images.append(example_image)
            # 使用confidence_score作为标签
            labels.append(example['confidence_score'])
...
def custom_collate(batch):
    ...
    for item in batch:
        query_images.extend(item['query_image'])
        example_images.extend(item['example_image'])
...
def parse_args():
    ...
    parser.add_argument('--candidate_num', default=32, type=int)
...
def custom_loss(outputs, labels, num_pairs):
    ...
    for i in range(num_query):
        ...
        print(f"高均值: {above_mean_mean:.4f}, 低均值: {below_mean_mean:.4f}, 差值: {difference:.4f}")
...
if __name__ == '__main__':
    ...
    # 创建输出目录
    os.makedirs(os.path.join(args.output_root, args.BENCHMARK), exist_ok=True)

    # 初始化数据集和模型
    dataset = ClassificationDataset(args.json_file, args.image_dir, num_pairs=args.candidate_num)
