from policy_model_eval_classification import parse_args as eval_parse_args, load_trained_policy_model
from policy_model_eval_classification import TinyImageNetDataset, evaluate_policy_model, custom_collate
from policy_model_score_classification import main as score_main, parse_args as score_parse_args
...
def main():
    ...
    # 加载数据集
    print("加载数据集...")
    val_dataset = TinyImageNetDataset(eval_args.root_dir, split="val")
    train_dataset = TinyImageNetDataset(eval_args.root_dir, split="train")
    ...
    eval_result_path = os.path.join(eval_args.output_path, f"eval_results_{timestamp}.json")
    ...
    score_args.output_file = os.path.join(eval_args.output_path, f"classification_results_{timestamp}.json")
    score_args.gpu = eval_args.gpu  # 使用相同的GPU
    score_args.shot_number = eval_args.shot_number  # 使用相同的示例数量
    score_args.root_dir = eval_args.root_dir  # 使用相同的数据集路径
